...
# Last Updated: <2020-May-06 14:22:25>
#

scriptdir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
tooldir=${scriptdir}/../project/tool/target
hc_classpath="${tooldir}/hashcash-tool-20200506.jar:$tooldir/lib/*"
resource=""
bits=""
hashFn="SHA-256"
cash=""
...
function usage() {
  ...
  echo "generate a hash with SHA-1: "
  echo "  $CMD -r dchiesa@google.com -b 20 -f SHA-1"
  echo "parse a hash: "
  ...
  echo "  -f hashFn     Optional. SHA-1 or SHA-256, for example. Defaults to SHA-256."
  echo "  -c hashcash   Optional. hashcash to parse."
  echo
  exit 1
}
